### Harvest Moon Tilemap Decompressor
### 0.1 - 2023

# This program compresses or uncompresses text files used as tilemaps on the
# game Harvest Moon for SNES. Values are separated by comas. End of lines
# might be added or read, when decompressing, but they are ignored, only for
# formatting
...
def main(argv):
    ...
    for opt, arg in opts:
        if opt == "-h":
            print ("test.py [-c] [-d] input_file output_file")
            sys.exit()
        elif opt == "-c":
            decomp = False
        elif opt == "-d":
